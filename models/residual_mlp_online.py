...
class ResidualMLPOnline(BaseModel):
    ...
    def predict(self, X: torch.Tensor, eval: bool=False) -> torch.Tensor:
        ...
        for i in range(36):
            self.scalars[i] *= fit_ransac_line(X[mask, i].cpu().numpy().astype(np.float32), references[mask, i].cpu().numpy().astype(np.float32), threshold=0.1, max_iters=50, seed=self.seed)
