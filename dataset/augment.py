...
def reconstruct_rss_lambertian(rss_ref, d1, d2, m = 22.9937273586):
    r"""
...
def augment_data(data: npt.NDArray[np.float64], fill_value=-1, q: int = 1) -> npt.NDArray[np.float64]:
    ...
    for y, x, no_led in tqdm(zip(y_indices, x_indices, l_indices), total=len(y_indices), desc=f"Augmenting data q={q}/4"):
        nearest_y = np.clip(int(np.round(y / 4) * 4), 0, data.shape[0] - 1)
        nearest_x = np.clip(int(np.round(x / 4) * 4), 0, data.shape[1] - 1)

        d1 = np.linalg.norm(leds[no_led] - np.array([x, y, 0]))
        d2 = np.linalg.norm(leds[no_led] - np.array([nearest_x, nearest_y, 0]))
...
def main():
    ...
    upsampled_q1 = augment_data(upsampled_q1, q=1)
    upsampled_q2 = augment_data(upsampled_q2, q=2)
    upsampled_q3 = augment_data(upsampled_q3, q=3)
    upsampled_q4 = augment_data(upsampled_q4, q=4)
