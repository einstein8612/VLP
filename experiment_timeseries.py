...
def generate_test_set(data: npt.NDArray, valid_mask: npt.NDArray, test_set_fraction: float, rng: np.random.Generator) -> tuple[npt.NDArray, npt.NDArray]:
    ...
    # Get samples
    samples = flat_data[sample_flat_idxs, led_ids] # Get the samples for each LED at each timestep with the same sample index. Shape (H*W*fraction, leds_n)
...
def generate_aged_samples(data: npt.NDArray, valid_mask: npt.NDArray, relative_decay: npt.NDArray, samples_per_timestep: int, rng: np.random.Generator) -> tuple[npt.NDArray, npt.NDArray]:
    ...
    sample_flat_idxs = rng.choice(valid_flat_idxs, size=(timesteps_n, samples_per_timestep))
    ...
    # Fetch the samples for each LED at each time step
    sample_flat_idxs = sample_flat_idxs[:, :, None] # Add a new axis to the sample_flat_idxs to match the shape of led_ids
    sample_flat_idxs = np.broadcast_to(sample_flat_idxs, (timesteps_n, samples_per_timestep, leds_n))
    led_ids = np.broadcast_to(led_ids, (timesteps_n, samples_per_timestep, leds_n))
...
def main(args):
    ...
    aged_samples_X, aged_samples_y = generate_aged_samples(data, valid_mask, relative_decay, args.samples_per_timestep, rng)
    aged_samples_X, aged_samples_y = torch.tensor(aged_samples_X, dtype=torch.float), torch.tensor(aged_samples_y, dtype=torch.float)*10
    print(f"Generated {aged_samples_X.shape[0]*aged_samples_X.shape[1]} samples; {aged_samples_X.shape[1]} samples at {aged_samples_X.shape[0]} timesteps")
    
    test_X, test_y = generate_test_set(data, valid_mask, 0.2, rng)
    
    # Translate to PyTorch
    test_X, test_y = torch.tensor(test_X, dtype=torch.float), torch.tensor(test_y, dtype=torch.float)*10
    ...
    for i, t in bar:
        ...
        aged_samples_X_t, aged_samples_y_t = aged_samples_X[i, :, :], aged_samples_y[i, :, :]
        
        sample_predictions = model.predict(aged_samples_X_t)
        sample_average_error = torch.norm(sample_predictions - aged_samples_y_t, dim=1).mean().item()
        ...
        bar.set_description(f"Average error at {t} hours: {average_error:.2f} mm")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--load_model", type=str, required=True, help="Model to load")
    parser.add_argument(
